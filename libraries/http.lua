-- pulled from https://github.com/xXehub/gamesense-workshop-luas/blob/main/libraries/http.lua
-- port for melatonin
...
local string_lower, string_len = string.lower, string.len
...
do
	...
	local function find_sig(mdlname, pattern, typename, offset, deref_count)
        local mdlref = memory.get_module(mdlname)
		local raw_match = memory.read_pointer(mdlref + pattern) or error("signature not found", 2)
